...
class ModifiedModel(nn.Module):
    ...
    def forward(self, input_data, phase_id=None):
        base_output = self.base_model(input_data)
        if self.enable_custom_layer and phase_id is not None:
            return self.custom_layer(base_output, phase_id)
        return base_output
...
def create_gentuo_fragments():
    ...
    return [lincoln_fragment, vajrayogini_fragment, vajrapani_fragment, duofragment]

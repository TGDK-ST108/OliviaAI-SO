...
def load_from_ox_file(ox_file_path='ox/config.ox', key=None):
    try:
        if key is None:
        ...
        logging.info(f"Configuration successfully loaded from '{ox_file_path}'.")
        return config
    except Exception as e:
        logging.error(f"Failed to load configuration from '{ox_file_path}': {e}")
        raise

def load_flask_secret_key(ox_file_path='ox/flask_secret_key.ox', key=None):
    try:
        if key is None:
            # Load key from file if not provided
            key = load_encryption_key()
        ...
        # Read the encrypted Flask secret key from the .ox file
        with open(ox_file_path, 'rb') as ox_file:
            encrypted_secret_key = ox_file.read()

        # Decrypt the secret key string
        decrypted_secret_key = fernet.decrypt(encrypted_secret_key).decode().strip()

        if not decrypted_secret_key:
            logging.error(f"Invalid Flask secret key format in '{ox_file_path}'")
            raise ValueError("The Flask secret key does not have a valid format.")

        logging.info(f"Flask secret key successfully loaded from '{ox_file_path}'.")
        logging.debug(f"Decrypted Flask secret key: {decrypted_secret_key}")
        return decrypted_secret_key
    except Exception as e:
        logging.error(f"Failed to load Flask secret key from '{ox_file_path}': {e}")
        raise
...
class SteelOx:
    def __init__(self, config, key_vault_uri, network_ranges, resource_id, flask_secret_key):
        self.config = config
        ...
        self.adaptive_firewall = AdaptiveFirewallSystem(config=config, flask_secret_key=flask_secret_key)
...
if __name__ == "__main__":
    ...
    try:
        ...
        key_vault_uri = key_vault.get("uri")
        network_ranges = config.get("network_ranges")
        quantum_workspace = config.get("quantum_workspace")
        resource_id = quantum_workspace.get("resource_id")
        ...
        try:
            ...
            logger.info("Azure Quantum Provider initialized successfully.")
        except Exception as e:
            logger.error(f"Failed to initialize Azure Quantum Workspace or Provider: {e}")
            sys.exit(1)

        # Initialize SteelOx with the required parameters
        steelox = SteelOx(
            config=config,
            key_vault_uri=key_vault_uri,
            network_ranges=network_ranges,
            resource_id=resource_id,
            flask_secret_key=config.get("flask_secret_key")  # Ensure Flask secret key is passed
        )

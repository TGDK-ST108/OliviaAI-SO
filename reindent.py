...
def quadrolineated_space_fragmenter(filename, base_indent_size=4):
    ...
    for line in lines:
        ...
        if re.match(r"^(class|def|if|for|while|try|with)\b", stripped_line):
            ...
            indent_level += 1  # Increase indent level for new block
        elif stripped_line.startswith("elif") or stripped_line.startswith("else") or stripped_line.startswith("except"):
            indent_level -= 1  # Align elif/else/except with previous if/try
            current_indent = ' ' * (base_indent_size * indent_level)
            indent_level += 1  # Restore level after else/elif/except
        elif stripped_line == "":
        ...
        if stripped_line.startswith("return") or stripped_line.startswith("pass") or stripped_line.startswith("continue") or stripped_line.startswith("break"):
            indent_level -= 1
    ...
    with open(filename, 'w') as file:
        for line in lines:
            ...
            if stripped_line.endswith(":") and stripped_line.split()[0] not in ['class', 'def']:
                final_fragment_indent = ' ' * base_indent_size
